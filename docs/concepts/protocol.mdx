...
Emitted when a session is initialized. Contains metadata about the active model, provider, and available skills/subagents.

```json
{
  "event": {
    "SessionStart": {
      "model": { "name": "claude-sonnet-4-5-20250514", "max_tokens": 8192 },
      "provider": "Anthropic",
      ...
      "subagents": [
        { "name": "explore", "description": "Explore the codebase", "color": "#4A90D9" }
      ]
    }
  }
}
...
| `skills` | `SkillMetadata[]` | Available skills |
| `subagents` | `SubagentMetadata[]` | Available subagents |
...
| Variant | Fields | Description |
|---|---|---|
| `Init` | `engine_status`, `system_caps`, `local_models`, `verified_models` | Initialization status report |
...
| `ServerReady` | `port: u16`, `model_name: string` | Offline server is ready to accept requests |
...
For external clients, `StdioTransport` bridges JSON Lines over stdin/stdout to the internal channel pair:
