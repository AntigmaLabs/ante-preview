...
- **Transport** — Bounded async channels (Tokio) for in-process clients (TUI, headless), or JSONL over stdin/stdout for external clients (`ante serve`). Message IDs enable tracing across the boundary.
