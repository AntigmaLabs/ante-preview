...
<Note>
WebSearch is not in the default tool set. Enable it via `--allowed-tools` or `--simple-agent` mode.
</Note>
...
- **Approval required**: No
- **Opt-in**: Not in the default tool set. Enable via `--allowed-tools` or `--simple-agent` mode.
...
- **Approval required**: No
- **Opt-in**: Only available via `--simple-agent` or `--allowed-tools`.
