...
The following tools are enabled by default: `Read`, `Bash`, `BashOutput`, `KillShell`, `Grep`, `Glob`, `TodoWrite`, `WebFetch`, `Edit`, `Write`, `Agent`. When the binary is built with the `browser` feature, `Browser` is also enabled by default.
...
Drive a Chromium browser for navigation, content extraction, and automation. Only available when Ante is built with the `browser` cargo feature.

- **Approval required**: Yes
- **Key inputs**: action-specific (e.g. `navigate`, `click`, `read_page`, `screenshot`)
