...
| `include_skills` | Skill names equipped on top of the session's base skill set — additive, like `--include-tools` for tools. Names match exactly |
| `exclude_skills` | Skill names removed from every session's skill set. Wins over `include_skills`; names match exactly. See [Selecting individual skills](/extend/skills#selecting-individual-skills) |
